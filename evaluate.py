...
def plot_qrtd(instance_name, qrtd_data, colors=None):
    ...
    for q in sorted(qrtd_data.keys()):
        ...
        plt.plot(times, fractions, label=f'q* = {q}%', color=colors.get(q, 'gray'))
...
def plot_sqd(problem_name, sqd_data, colors=None):
    ...
    if not colors:
        # Create colors for different time points
        cmap = plt.cm.viridis
...
def main():
    ...
    for key in plot_list:
        ...
        # Calculate SQD
        sqd_time_points = [max_time * i / 10 for i in range(1, 11)]  # 10%, 20%,... of max time
        sqd_results = {t: [] for t in sqd_time_points}
        
        for time_point in sqd_time_points:
            q_values = np.linspace(0, 10, 51)  # 0% to 10% from optimal
            fractions = []
